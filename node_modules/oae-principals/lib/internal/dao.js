 ...
 * Create share code 
 *
 * @param  {String}         groupId             The id the group
 * @param  {String}         role                The role that users will have thanks to the code
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  [shareCode]      callback.shareCode  The details of shareCode
 * @api private
 */
var createShareCode = module.exports.createShareCode = function(groupId, role, callback) {
    var randomCode = generateCode(30)
    var queries = [];

    queries.push({'query': 'INSERT INTO "ShareCodeByGoupId" ("groupId", "code", "role") VALUES (?, ?, ?)', 'parameters': [groupId, randomCode, role]});
    queries.push({'query': 'INSERT INTO "ShareCode" ("code", "groupId", "role") VALUES (?, ?, ?)', 'parameters': [randomCode, groupId, role]});
 ...
 * @api private
 */
var getShareCodeByGoupId = module.exports.getShareCodeByGoupId = function(groupId, callback) {
    var query = 'SELECT * FROM "ShareCodeByGoupId" WHERE "groupId" = ?';

    Cassandra.runQuery(query, [groupId], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback(null, []);
 ...
 *
 * @param  {String}         code                The code of the share code
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  [User]           callback.users      The list of users for the given tenancy
 * @api private
 */
var getShareCode = module.exports.getShareCode = function(code, callback) {
    var query = 'SELECT * FROM "ShareCode" WHERE "code" = ?';

    Cassandra.runQuery(query, [code], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback(null, []);
 ...
 * @api private
 */
var removeShareCode = module.exports.removeShareCode = function(groupId, callback) {

    getShareCodeByGoupId(groupId, function(err, shareCode) {
        if (err) {
            return callback(err);
        }
        if (_.isEmpty(shareCode)) {
            return callback(null);
        }

        var queries = [];

        queries.push({'query': 'DELETE FROM "ShareCodeByGoupId" WHERE "groupId" = ?', 'parameters': [groupId]});
        queries.push({'query': 'DELETE FROM "ShareCode" WHERE "code" = ?', 'parameters': [shareCode.code]});
 ...
 * Generate a code
 *
 * @param  {Number}       x                   The length of the generated code
