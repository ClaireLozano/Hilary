...
/**
 * @REST postGroupGroupIdRoleCreateShareCode
 *
 * Create share code
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/{role}/createShareCode
 * @PathParam   {String}        groupId                 The id the group
 * @PathParam   {string}        role                    The role that users will have thanks to the code
 * @Return      {shareCode}                             The share code created
 * @HttpResponse                200                     The share code created
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/:role/createShareCode', function(req, res) {

    PrincipalsAPI.createShareCode(req.ctx, req.params.groupId, req.params.role, function(err, shareCode) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).send(shareCode);
    });
});

/**
 * @REST getGroupGroupIdShareCode
 *
 * Get share code details
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/shareCode
 * @PathParam   {String}        groupId                 The id the group
 * @PathParam   {string}        role                    The role that users will have thanks to the code
 * @Return      {shareCode}                             The share code generated
 * @HttpResponse                200                     The share code details
 */
OAE.tenantRouter.on('get', '/api/group/:groupId/shareCode', function(req, res) {

    PrincipalsAPI.getShareCodeByGoupId(req.ctx, req.params.groupId, function(err, shareCode) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 * @PathParam   {String}        groupId                 The id the group
 * @Return      {}                             
 * @HttpResponse                200                     Share code removed
 */
OAE.tenantRouter.on('delete', '/api/group/:groupId/shareCode', function(req, res) {
    PrincipalsAPI.removeShareCode(req.ctx, req.params.groupId, function(err, shareCode) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
/**
 * @REST postGroupJoinCode
 *
 * Join the group by the code
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/join/{code}
 * @PathParam   {string}        code                    The code given by the user to join the group
 * @Return      {}                             
 * @HttpResponse                200                     The user has join the group
 */
OAE.tenantRouter.on('post', '/api/group/join/:code', function(req, res) {
    
    PrincipalsAPI.joinGroupWithCode(req.ctx, req.params.code, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
