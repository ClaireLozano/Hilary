 ...
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occured, if any
 * @param  {Boolean}    callback.shareCode          The details of the share code
 * @api private
 */
var createShareCode = module.exports.createShareCode = function(ctx, groupId, role, callback) {
    // Anonymous users cannot manage anything
    if (!ctx.user()) {
        return callback(null, false);
    }

    PrincipalsDAO.createShareCode(groupId, role, function(err, shareCode) {
        if (err) {
            return callback(err);
        }

        return callback(null, shareCode);
    });
};

/**
 * Get share code details by groupId
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
 * @param  {String}     groupId                     The group id
 * @param  {Function}   callback                    Standard callback function
 ...
 * @api private
 */
var getShareCodeByGoupId = module.exports.getShareCodeByGoupId = function(ctx, groupId, callback) {
    // Anonymous users cannot manage anything
    if (!ctx.user()) {
        return callback(null, false);
    }

    PrincipalsDAO.getShareCodeByGoupId(groupId, function(err, shareCode) {
        if (err) {
            return callback(err);
        }
        return callback(null, shareCode);
...
/**
 * Get share code details by code
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
 * @param  {String}     groupId                     The group id
 * @param  {Function}   callback                    Standard callback function
 ...
 * @api private
 */
var getShareCode = module.exports.getShareCode = function(ctx, groupId, callback) {
    // Anonymous users cannot manage anything
    if (!ctx.user()) {
        return callback(null, false);
    }

    PrincipalsDAO.getShareCode(groupId, function(err, shareCode) {
        if (err) {
            return callback(err);
        }
        return callback(null, shareCode);
 ...
 * @api private
 */
var removeShareCode = module.exports.removeShareCode = function(ctx, groupId, callback) {
    // Anonymous users cannot manage anything
    if (!ctx.user()) {
        return callback(null, false);
    }
    
    PrincipalsDAO.removeShareCode(groupId, function(err) {
        if (err) {
            return callback(err);
        }
...
/**
 * Join the group with a code
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
 * @param  {String}     code                        The code given by the user to join the group
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occured, if any
 * @param  {Boolean}    callback.shareCode          The details of the share code
 * @api private
 */
var joinGroupWithCode = module.exports.joinGroupWithCode = function(ctx, code, callback) {
    ...
    PrincipalsDAO.getShareCode(code, function(err, shareCode) {
        if (err) {
            return callback(err);
        }

        AuthzAPI.hasAnyRole(ctx.user().id, shareCode.groupId, function(err, hasRole) {
            if (err) {
                return callback(err);
            }
            if (hasRole) {
                return callback({'code': 400, 'msg': 'You have already rights on this group'});
            }

            if (shareCode.code === code) {

                // Check if the group exists
                PrincipalsDAO.getPrincipal(shareCode.groupId, function(err, group) {
                    if (err) {
                        return callback(err);
                    }

                    // Update role
                    AuthzAPI.updateRoles(shareCode.groupId, _.oaeObj(ctx.user().id, shareCode.role), function(err, usersToInvalidate) {
                        if (err) {
                            return callback(err);
                        }

                        AuthzPermissions.validateRoleChanges(ctx, group, [ctx.user()], {'promoteOnly': true}, function(err, memberChangeInfo, emailChangeInfo) {
                            if (err) {
                                return callback(err);
                            }
                ...
                return callback({'code': 400, 'msg': 'Wrong code'});
            }
        });
    });
};
